## Done - TWO paramters to be Edited
...
from utils.augmentation import between_class, mixup_criterion
from utils.customdatasets import CoffeeLeavesDataset
from utils.utils import static_graph, plot_confusion_matrix
from net_models import resnet34, resnet50, resnet101, alexnet, googlenet
...
clf_label = [ 'leaf_multitask' , 'leaf_disease', 'leaf_severity']

def cnn_model(model_name, pretrained=False, num_classes=(5, 5)):  ## to be Edited
...
class MultiTaskClf:
    ...
    def train(self, train_loader, model, criterion, optimizer, data_augmentation=None):
        ...
        for images, labels_dis, labels_sev, img_id in train_loader:
    ...
    def validation(self, val_loader, model, criterion):
        ...
        with torch.no_grad():
            for images, labels_dis, labels_sev, img_id in val_loader:
                # Loading images on gpu
                if torch.cuda.is_available():
                    images, labels_dis, labels_sev = images.cuda(), labels_dis.cuda(), labels_sev.cuda()
    ...
    def run_training(self):
        assert isinstance(self.opt.filename, object)
    ...
    def run_test(self):
        ...
        with torch.no_grad():
            for i, (images, labels_dis, labels_sev, img_id) in enumerate(test_loader):
                # Loading images on gpu
                if torch.cuda.is_available():
                    images, labels_dis, labels_sev, img_id = images.cuda(), labels_dis.cuda(), labels_sev.cuda(), img_id.cuda()
                ...
                #### Write the predection into csv file
                y_pred_dis_list.append(pred.tolist()) # to collect predictions
                ...
                #### Write the predection into csv file
                y_pred_sev_list.append(pred.tolist())


                imge_name_list.append(img_id.tolist())

                # f = open('results/' + clf_label[self.opt.select_clf] + '/' + self.opt.filename + '_pred.csv', 'a')
                # f.write('Image id, Biotic stress, Severity\n%.2f,%.2f,%.2f,%.2f\n' % ("try", y_pred_dis, y_pred_sev))

        # Biotic stress
        # labels_dis = ['Healthy', 'Leaf miner', 'Rust', 'Phoma', 'Cercospora']
        # replace y_pred_dis_list with Disease Names
        for i in range(len(y_pred_dis_list)):
            if y_pred_dis_list[i] == 0 :
                y_pred_dis_list[i] = 'Healthy'
            elif y_pred_dis_list[i] == 1 :
                y_pred_dis_list[i] = 'Leaf miner'
            elif y_pred_dis_list[i] == 2 :
                y_pred_dis_list[i] = 'Rust'
            elif y_pred_dis_list[i] == 3 :
                y_pred_dis_list[i] = 'Phoma'
            elif y_pred_dis_list[i] == 4 :
                y_pred_dis_list[i] = 'Cercospora'

        # Severity
        # replace y_pred_sev_list with Severity Conditions
        # labels_sev = ['Healthy', 'Very low', 'Low', 'High', 'Very high']
        # for i in range(len(y_pred_sev_list)):
        #     if y_pred_sev_list[i] == 0 :
        #         y_pred_sev_list[i] = 'Healthy'
        #     elif y_pred_sev_list[i] == 1 :
        #         y_pred_sev_list[i] = 'Very low'
        #     elif y_pred_sev_list[i] == 2 :
        #         y_pred_sev_list[i] = 'Low'
        #     elif y_pred_sev_list[i] == 3 :
        #         y_pred_sev_list[i] = 'High'
        #     elif y_pred_sev_list[i] == 4 :
        #         y_pred_sev_list[i] = 'Very high'
        ...
        def flatten(lis):
            for item in lis:
                if isinstance(item, Iterable) and not isinstance(item, str):
                    ...
                    yield item
        y_pred_dis_list = list(flatten(y_pred_dis_list))
        y_pred_sev_list = list(flatten(y_pred_sev_list))
        imge_name_list = list(flatten(imge_name_list))
        print(y_pred_dis_list)
        print(y_pred_sev_list)
        print(imge_name_list)

        # pred_df = pd.DataFrame(zip(imge_name_list,y_pred_dis_list, y_pred_sev_list),
        #                   columns=['img_id','Leaf Disease', 'Leaf Severity'])
        pred_df = pd.DataFrame({
                    'img_id'        : imge_name_list,
                    'Leaf Disease'  : y_pred_dis_list,
                    'Leaf Severity' : y_pred_sev_list
        })
        pred_df.to_csv('results/' + clf_label[self.opt.select_clf] + '/' + self.opt.filename + '.csv', index=False)
...
class OneTaskClf: ## to be Edited (( deliver prediction as csv file at "run_test" function ))
    def __init__(self, parser):
        self.opt = parser.parse_args()

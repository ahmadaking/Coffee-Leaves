...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_augmentation",
        type=str,
        help="Select the data augmentation technique [standard/bc+]",
        default="standard",
    )
    parser.add_argument(
        "--model",
        type=str,
        help="Select CNN architecture [resnet50/resnet101/googlenet]",
        default="resnet50",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--select_clf",
        help="Select the experiment to run. Leaf dataset: (0) multitask, (1) biotic stress only, (2) severity only.",
        type=int,
        default=0,
    )
